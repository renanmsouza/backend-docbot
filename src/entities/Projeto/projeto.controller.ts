...
export class ProjetoController {
    ...
    public async findAll(@Res() res: Response): Promise<Response> {
        try {
            return res.status(200)
                .send(new Resposta('Sucesso', 'Todos os Resultados', await this.projetoService.findAll()));    
        } catch (error) {
    ...
    public async findById(@Param('id') id: number, @Res() res: Response): Promise<Response> {
        try {
            return res.status(200).send(new Resposta('Sucesso', 'Pesquisa por ID', [await this.projetoService.findById(id)]))
        } catch (error) {
    ...
    public async salvar(@Req() req: Request, @Res() res: Response): Promise<Response> {
        try {
            const data: Projeto = req.body as Projeto;

            await this.projetoService.save(data);
